 ...
 * 장바구니 API 명세 검증.
 *
 * <p>내부 클래스 구조가 아니라 HTTP 계약(경로, 상태 코드, 응답 JSON)만 검증한다.
...
class CartApiTest {
    ...
    class Find {
        ...
        void emptyCartReturnsOk() throws Exception {
            mockMvc.perform(get(CART).header(USER, 1))
                    .andExpect(status().isOk())
        ...
        @Test
        @DisplayName("항목에 상품명·단가·소계가 함께 내려온다")
        void itemContainsProductInfo() throws Exception {
            ...
            mockMvc.perform(get(CART).header(USER, 1))
                    ...
                    .andExpect(jsonPath("$.items[0].price").value(68000))
                    .andExpect(jsonPath("$.items[0].quantity").value(2))
                    .andExpect(jsonPath("$.items[0].subtotal").value(136000));
        }

        @Test
        @DisplayName("총 수량과 총액을 서버가 계산해 내려준다")
    ...
    class Add {
        ...
        void addingSameProductTwiceMergesQuantity() throws Exception {
            ...
            Integer rows = jdbcTemplate.queryForObject(
                    "SELECT COUNT(*) FROM cart_item WHERE cart_id = 1 AND product_id = 1", Integer.class);
    ...
    class UpdateQuantity {
        ...
        void quantityIsUpdated() throws Exception {
            ...
            mockMvc.perform(patch(ITEMS + "/" + itemId)
                    ...
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.items[0].quantity").value(5))
                    .andExpect(jsonPath("$.items[0].subtotal").value(340000))
    ...
    class Remove {
        ...
        void removeReturnsNoContent() throws Exception {
            ...
            mockMvc.perform(delete(ITEMS + "/" + itemId).header(USER, 1))
        ...
        void unknownItemIsNotFound() throws Exception {
            mockMvc.perform(delete(ITEMS + "/999999").header(USER, 1))
                    .andExpect(status().isNotFound())
                    .andExpect(jsonPath("$.code").value("NOT_FOUND"));
        }
        ...
        void otherMembersItemIsNotFound() throws Exception {
            ...
            mockMvc.perform(delete(ITEMS + "/" + otherItemId).header(USER, 1))
                    .andExpect(status().isNotFound());
