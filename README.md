...
### 조건부 UPDATE 를 한 곳에만 적용했더니 취소가 뚫렸다

- **상황** — 재고 차감은 조건부 UPDATE 로 막아 뒀으니 정합성은 끝났다고 생각했다.
  동시성 테스트를 써 보니 **주문 취소가 전혀 막히지 않았다.**
  같은 주문을 10개 스레드가 동시에 취소하자 10건 모두 성공했고,
  재고는 100 에서 145 가 됐다. 판 적 없는 재고 45 개가 생긴 셈이다.
- **원인** — 취소는 읽고-검사하고-쓰는 세 단계였다.

  ```java
  Order order = getOrThrow(memberId, orderId);  // ① SELECT (스냅샷)
  order.cancel();                                // ② 자바에서 PENDING 인지 검사
  orderMapper.update(order);                     // ③ UPDATE ... WHERE id = ?
  ```

  ① 의 잠금 없는 읽기는 트랜잭션마다 자기 시작 시점의 스냅샷을 본다.
  10 개가 전부 `PENDING` 을 읽고 ② 를 통과한다.
  ③ 은 상태를 다시 보지 않으므로 아무도 막히지 않는다.
- **해결** — 검사 조건을 `WHERE` 로 옮겨 한 문장으로 합쳤다.

  ```sql
  UPDATE orders SET status = 'CANCELLED'
   WHERE id = #{id} AND member_id = #{memberId} AND status = 'PENDING'
  ```

  `UPDATE` 는 `SELECT` 와 달리 스냅샷이 아니라 **최신 커밋 버전**을 읽는다.
  먼저 잠금을 잡은 트랜잭션이 커밋하면, 기다리던 쪽은 깨어나 `WHERE` 를 다시 평가하고
  0 행이 영향받는다. 재고 차감이 동작하는 원리와 같다.
- **배운 것** — 정합성 패턴은 **한 군데 적용했다고 끝이 아니다.**
  상태를 바꾸는 모든 지점에 같은 질문을 해야 한다.
  그리고 이건 단위 테스트로는 절대 드러나지 않는다.
  스레드를 실제로 여러 개 띄워 보기 전까지 이 코드는 90 개 테스트를 통과하고 있었다.

### 동시성 테스트를 쓸 때 조심할 것

- **클래스에 `@Transactional` 을 붙이면 안 된다.** 테스트가 만든 픽스처가 커밋되지 않아
  다른 스레드에서는 아예 보이지 않는다. 자동 롤백을 포기하고 직접 치워야 한다.
- **스레드를 그냥 `submit` 하면 경합이 일어나지 않을 수 있다.**
  앞 스레드가 이미 끝난 뒤 뒤 스레드가 시작하기 때문이다.
  `CountDownLatch` 로 전부 대기시켰다가 한 번에 출발시킨다.
  이걸 빠뜨리면 테스트가 조용히 아무것도 검증하지 않는다.
